...
sourceCsv = os.path.join( directory , filepath.replace("\\adult_","\\").replace("\\child_","\\")+".csv")
...
#        obj.pose.bones[key].location = desiredLoc
        ...
        obj.pose.bones["b__ROOT_bind__"].location[2] = obj.pose.bones["b__ROOT_bind__"].location[2] - diff[0] + 0.00 #You can adjust the added/subtracted values to make adjustments in positioning to the entire animation
